...
const InputWithLabel = () => {
    ...
    const handleClick = () => {
        navigator.clipboard.writeText(InputWithLabel)
        setIsCopied(true);
        setTimeout(() => {
            setIsCopied(false);
        }, 2000)
            ...
            <div className={`w-full border min-h-72 py-6 relative flex items-center justify-center rounded-lg`}>
                {
                    isActive == "preview" ?
                        <div className="flex flex-col items-start gap-1">
                            <label htmlFor="input" className="text-[0.8rem]">Enter your email</label>
                            <input type="text" name="input" className="border py-2 px-3 w-72 bg-transparent rounded-lg text-[0.85rem]" placeholder="sabin@gmail.com" />
                        </div>
